...
using WebAPI_Axess.Ramundberget;
...
{
    /// <summary>
    ///This class represent a Web API that is used to get message from a ski resort.
    ///The transactions that is received is of two types.
    ///1. The first one is when some buy ski pass.
    ///2. When somebody blipp ski pass
    /// </summary>
    public class RamundbergetController : Controller
    {
        string username = "QBIM";
        string password = "F9sUw!ho";
        string connectionString = "Data Source=qbim.database.windows.net;Initial Catalog=Ramundberget;Persist Security Info=True;User ID=Daniel;Password=QbimAdmin123%";

        AxDCI4CRMClient client = new AxDCI4CRMClient("BasicHttpsBinding_IAxDCI4CRM2");

        /// <summary>
        /// Default method that is called on start of Web API
        /// </summary>
        ...
        /// <summary>
        /// This method will call a SOAP API to get an object containing transactions to buy lift pass.
        /// It will than call helpers to process and store the data in a database
        /// </summary>
        /// <param name="fromRow">Represent the point from where to start to read in the external SOAP API</param>
        /// <param name="sessionId">Is used to identify a network communication</param>
        /// <returns>It will return as Json what method InsertSales returns </returns>
        [HttpGet]
        public JsonResult TransferSales(string fromRow, string sessionId)
        {
            try
            {
                ...
                //Handle and process the result into db
        ...
        /// <summary>
        /// Handle the processing of inserting values into a db
        /// </summary>
        /// <param name="result">Represent an object containing all the transactions about lift pass</param>
        /// <returns>Return an object of class ResponseMessage </returns>
        public ResponseMessage InsertSales(DCI4CRMLOGLISTRESULT result)
        {
            ...
            // Define the complete sql qyuery
            ...
            //Configure the SqlConnection and SqlCommand 
            ...
            using (SqlCommand command = new SqlCommand(insertQuery, connection))
            {
                ...
                command.Parameters.Add("@fEinzelTarifTats", SqlDbType.Float);
                command.Parameters.Add("@fEinzelTarifBlatt", SqlDbType.Float);
                ...
                command.Parameters.Add("@fAmount", SqlDbType.Float);
                command.Parameters.Add("@nSaleQuantity", SqlDbType.Int);
                command.Parameters.Add("@fWareTarif", SqlDbType.Float);
                command.Parameters.Add("@nLogNumber", SqlDbType.Int);

                //Open a connection to the database
                connection.Open();

                //Loop through all the transactions in the passed object
                foreach (var saleItem in result.ACTLOGLINE)
                {
                    try
                    {
                        //CreateParameterValue will get the value for each passed key
                        ...
                        command.Parameters["@fEinzelTarifTats"].Value = CreateParameterValue(saleItem, "FEINZELTARIFTATS");
                        command.Parameters["@fEinzelTarifBlatt"].Value = CreateParameterValue(saleItem, "FEINZELTARIFBLATT");
                        ...
                        command.Parameters["@fAmount"].Value = CreateParameterValue(saleItem, "FAMOUNT");
                        command.Parameters["@nSaleQuantity"].Value = CreateParameterValue(saleItem, "NSALEQUANTITY");
                        command.Parameters["@fWareTarif"].Value = CreateParameterValue(saleItem, "FWARETARIF");
                        command.Parameters["@nLogNumber"].Value = int.Parse(saleItem.NLOGNR.Value.ToString());

                        //Execute the query which mean insert into db
                        command.ExecuteNonQuery();

                        //Is the number of transfered transactions
                        numberOfTransferedRows++;

                        //Is the running number of transactions 
                        lastRowNumber = int.Parse(saleItem.NLOGNR.Value.ToString());
                    }
                    catch (Exception ex)
                    {
                        //In case of somebody throws an error this object is instansiated
                ...
                //Return and instansiate a new object of class ResponseMessage
                return new ResponseMessage()
                {
                    LastRow = lastRowNumber,
                    NumberOfTransferedRows = numberOfTransferedRows,
        ...
        /// <summary>
        /// Is used to find the value for the passed key in the passed transaction
        /// </summary>
        /// <param name="saleItem">The object that contains a transactions for lift pass</param>
        /// <param name="key">The key that is used to find the value in the passed object</param>
        /// <returns>If key is found return the value else return DBNull</returns>
        ...
        /// <summary>
        /// Is used to find the value for the passed key in the passed transaction
        /// This method is special to handle just date
        /// </summary>
        /// <param name="saleItem">The object that contains a transactions for lift pass</param>
        /// <param name="key">The key that is used to find the value in the passed object</param>
        /// <returns>If key is found return the value else return DBNull</returns>
        ...
        /// <summary>
        /// This method will call a REST API to get an object containing  transactions for blippa lift card.
        /// It will than call helpers to process and store the data in a database
        /// </summary>
        /// <param name="fromRow">Represent the point from where to start to read in the external REST API</param>
        /// <param name="sessionId">Is used to identify a network communication</param>
        /// <returns>It will return as Json what method InsertSales returns </returns>
        [HttpGet]
